...
int main() {
    ...
    // 提交日志任务
    auto logTask = pool.submit("logger", [](const std::string& msg) {
    ...
    // 提交网络任务
    auto networkTask = pool.submit("network", []() {
    ...
    for (int i = 0; i < 5; ++i) {
        generalTasks.push_back(pool.submit("general", [i]() {
            std::cout << getCurrentTime() << " [General] 任务 " << i << " 开始执行\n";
